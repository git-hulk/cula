...
func (s *session) spawnAndWait(prompt string) int {
	...
	cmd := exec.CommandContext(s.ctx, iruntime.BinaryPath(s.runtime.cfg, "copilot"), args...)
	...
	if err := cmd.Start(); err != nil {
		s.emitError(fmt.Sprintf("start copilot: %v", err))
		return 1
	}
...
func (s *session) buildArgs(prompt string) []string {
	args := []string{"--prompt", prompt, "--output-format", "json", "--no-ask-user"}
	if s.input.Permission != cula.PermissionNever {
		args = append(args, "--allow-all-tools")
	}
	if s.input.Sandbox == cula.SandboxDangerFullAccess {
		args = append(args, "--allow-all-paths", "--allow-all-urls")
	}
	if s.input.Model != "" {
	...
	if s.input.WorkingDir != "" {
		args = append(args, "--add-dir", s.input.WorkingDir)
	}
	return args
}
...
func (s *session) emitEvent(raw json.RawMessage, ev cula.Event) {
	ev.Runtime = cula.RuntimeCopilot
